...
func generateYAMLEnvFile(rc *eos_io.RuntimeContext, outputDir string, hecateSecrets *HecateSecrets) error {
	...
	// Generate .env content with real secrets
...
`,
		hecateSecrets.PGPass,
		hecateSecrets.PGUser,
		hecateSecrets.PGDatabase,
		...
		hecateSecrets.ComposePortHTTP,
		hecateSecrets.ComposePortHTTPS,
	)
...
func validateDockerCompose(rc *eos_io.RuntimeContext, outputDir string) error {
	...
	if err != nil {
		logger.Debug("Docker not found in PATH, skipping validation")
		return nil // Not an error - docker might not be installed yet
	}

	composeFile := filepath.Join(outputDir, "docker-compose.yml")

	logger.Debug("Validating docker-compose.yml",
		zap.String("docker_path", dockerPath),
		zap.String("compose_file", composeFile))

	// Run docker compose config --quiet to validate
	cmd := exec.Command(dockerPath, "compose", "-f", composeFile, "config", "--quiet")
	...
	logger.Debug("Docker compose validation successful")
	return nil
}
