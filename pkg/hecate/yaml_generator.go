...
{
	# Admin API binding (P0 FIX - Connection Reset)
...
{{$app.Domain}} {
	...
	{{if eq $app.Type "authentik"}}
	reverse_proxy http://hecate-server-1:9000

	{{else if and (eq $app.Type "bionicgpt") $.HasAuthentik}}
	# Self-service endpoints
	handle /signup {
		redir https://{{$.AuthentikDomain}}/if/flow/default-enrollment-flow/ 302
	}

	handle /reset-password {
		redir https://{{$.AuthentikDomain}}/if/flow/default-recovery-flow/ 302
	}
	...
	# BionicGPT logout handler (oauth2-proxy style)
	handle /oauth2/sign_out {
		header Set-Cookie "authentik_proxy=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Lax"
		redir https://{{$.AuthentikDomain}}/application/o/{{$name}}/end-session/?post_logout_redirect_uri=https://{{$app.Domain}}/ 302
	}
	...
	handle {
		...
		# Proxy to BionicGPT with explicit header mapping
...
networks:
  hecate-net:
    ...
    # EVIDENCE: User diagnostic showed "hecate_hecate-net" causing connection failures
    # VENDOR: Docker Compose docs recommend explicit names for shared networks
    name: hecate-net
    driver: bridge
