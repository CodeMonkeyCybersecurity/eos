...
func RunHecateDebug(rc *eos_io.RuntimeContext, cmd *cobra.Command, args []string) error {
	...
	if authentikCheck {
		return runAuthentikPreUpgradeCheck(rc, hecatePath)
	}
...
func runAuthentikPreUpgradeCheck(rc *eos_io.RuntimeContext, hecatePath string) error {
	logger := otelzap.Ctx(rc.Ctx)
	...
	allResults = append(allResults, checkAuthentikVersion(rc, hecatePath)...)

	// 2. Disk space check
	allResults = append(allResults, checkAuthentikDiskSpace(rc, hecatePath)...)

	// 3. Container health
	allResults = append(allResults, checkContainerHealth(rc, hecatePath)...)

	// 4. PostgreSQL checks
	allResults = append(allResults, checkPostgreSQLEncoding(rc, hecatePath)...)

	// 5. Redis check
	allResults = append(allResults, checkRedisConnectivity(rc, hecatePath)...)

	// 6. Custom modifications check
	allResults = append(allResults, checkCustomModifications(rc, hecatePath)...)

	// 7. Environment file check
	allResults = append(allResults, checkEnvironmentFile(rc, hecatePath)...)

	// 8. Task queue check
	allResults = append(allResults, checkTaskQueue(rc, hecatePath)...)

	// 9. Memory check
	allResults = append(allResults, checkMemoryUsage(rc, hecatePath)...)

	// 10. Backup status check
	allResults = append(allResults, checkBackupStatus(rc, hecatePath)...)
...
func checkAuthentikVersion(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
...
func checkAuthentikDiskSpace(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
...
func checkContainerHealth(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
...
func checkPostgreSQLEncoding(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
...
func checkRedisConnectivity(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
...
func checkCustomModifications(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
...
func checkEnvironmentFile(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
...
func checkTaskQueue(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
...
func checkMemoryUsage(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
...
func checkBackupStatus(rc *eos_io.RuntimeContext, hecatePath string) []HecateCheckResult {
	var results []HecateCheckResult
