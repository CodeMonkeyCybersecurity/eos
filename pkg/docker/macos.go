...
func CheckAndInstallDockerIfNeeded() error {
	...
	if !platform.IsCommandAvailable("brew") {
		...
		return crerr.New("Homebrew is required to install Docker. Visit https://brew.sh to install it.")
	}
	...
	if !shouldInstall {
		return crerr.WithHint(
			...
			"Please install Docker Desktop from https://www.docker.com/products/docker-desktop or via `brew install --cask docker`.",
		)
	}
