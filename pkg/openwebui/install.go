...
func (owi *OpenWebUIInstaller) createEnvFile(ctx context.Context) error {
	...
	var content string
	if owi.config.UseLiteLLM {
		// URL-encode the PostgreSQL password for use in DATABASE_URL
		// Special characters like # ? & ( ) : break URL parsing
		encodedPassword := url.QueryEscape(owi.config.PostgresPassword)
...
services:
  ...
  litellm-proxy:
    ...
    environment:
      # Use POSTGRES_PASSWORD_ENCODED (URL-encoded) to handle special characters like # ? & ( ) :
      DATABASE_URL: postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD_ENCODED}@litellmproxy_db:5432/${POSTGRES_DB}
