...
func (owi *OpenWebUIInstaller) performInstallation(ctx context.Context) error {
	...
	if owi.config.UseLiteLLM {
		...
		if owi.config.PostgresPassword == "" {
			...
			pgPassword, err := crypto.GenerateURLSafePassword(32)
			if err != nil {
				return fmt.Errorf("failed to generate PostgreSQL password: %w", err)
			}
			owi.config.PostgresPassword = pgPassword
			logger.Debug("Post-operation: PostgreSQL password generated (URL-safe)",
...
func (owi *OpenWebUIInstaller) createEnvFile(ctx context.Context) error {
	...
	if owi.config.UseLiteLLM {
		...
		// Note: PostgreSQL password uses URL-safe characters ([a-zA-Z0-9_-]) to avoid
		// issues with special characters in DATABASE_URL connection strings
...
services:
  ...
  litellm-proxy:
    ...
    environment:
      # PostgreSQL password uses URL-safe characters ([a-zA-Z0-9_-]) for DATABASE_URL compatibility
