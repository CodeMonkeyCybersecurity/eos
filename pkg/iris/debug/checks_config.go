// pkg/iris/debug/checks_config.go
package debug
...
// CheckConfiguration loads and validates the Iris configuration file
func CheckConfiguration(rc *eos_io.RuntimeContext, projectDir string, verbose bool) (*IrisConfig, CheckResult) {
	config, err := loadConfiguration(rc, projectDir, verbose)
	result := CheckResult{
...
func loadConfiguration(rc *eos_io.RuntimeContext, projectDir string, verbose bool) (*IrisConfig, error) {
...
// CheckAzureOpenAI validates the Azure OpenAI configuration
func CheckAzureOpenAI(rc *eos_io.RuntimeContext, config *IrisConfig) CheckResult {
	err := validateAzureOpenAI(config)
	result := CheckResult{
...
func validateAzureOpenAI(config *IrisConfig) error {
	if config == nil {
...
// CheckSMTPConfig validates the SMTP configuration
func CheckSMTPConfig(rc *eos_io.RuntimeContext, config *IrisConfig) CheckResult {
...
func validateSMTPConfig(config *IrisConfig) error {
	if config == nil {
		return fmt.Errorf("config not loaded")
	}
