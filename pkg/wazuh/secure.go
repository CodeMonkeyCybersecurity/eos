...
func RotateWithTool(rc *eos_io.RuntimeContext) error {
	_, span := telemetry.Start(rc.Ctx, "wazuh.rotate_tool")
	defer span.End()

	rc.Log.Info(" Downloading Wazuh password tool",
		zap.String("url", WazuhPasswdToolURL),
		zap.String("path", WazuhPasswdToolPath),
	)

	if err := utils.DownloadFile(WazuhPasswdToolPath, WazuhPasswdToolURL); err != nil {
		return cerr.Wrapf(err, "download tool")
	}
	if err := os.Chmod(WazuhPasswdToolPath, 0o700); err != nil {
...
func RunPrimary(rc *eos_io.RuntimeContext, apiPass string) (*bytes.Buffer, error) {
	...
	cmd := exec.CommandContext(rc.Ctx,
		filepath.Base(WazuhPasswdToolPath),
...
func RunFallback(rc *eos_io.RuntimeContext) (string, error) {
	...
	retry := exec.CommandContext(rc.Ctx, filepath.Base(WazuhPasswdToolPath),
...
func ParseSecrets(rc *eos_io.RuntimeContext, out *bytes.Buffer) map[string]string {
	_, span := telemetry.Start(rc.Ctx, "wazuh.parse_secrets")
	defer span.End()
