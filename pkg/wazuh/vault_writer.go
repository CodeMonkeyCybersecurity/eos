...
// WriteConfig writes Wazuh config to Vault (if available), and always to disk as fallback.
func WriteConfig(rc *eos_io.RuntimeContext, cfg *Config) error {
	...
	if err != nil {
		otelzap.Ctx(rc.Ctx).Warn("Failed to marshal Wazuh config", zap.Error(err))
	...
	otelzap.Ctx(rc.Ctx).Info("Wazuh config saved to disk", zap.String("path", diskPath))
	...
	if err := vault.Write(rc, nil, VaultWazuhConfig, cfg); err != nil {
		...
		otelzap.Ctx(rc.Ctx).Info("Wazuh config also saved to Vault", zap.String("vault_path", VaultWazuhConfig))
	}
	return nil
}
