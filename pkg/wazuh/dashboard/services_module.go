/* pkg/wazuh/dashboard/services_module.go */
...
// WazuhServices contains the list of all Wazuh pipeline services
// This should match the service registry in cmd/wazuh/services/service_registry.go
var WazuhServices = []string{
	"wazuh-listener",
...
func NewServicesModule(rc *eos_io.RuntimeContext, db *sql.DB) *ServicesModule {
	base := NewBaseModule("Services", ModuleServices, "Interactive management of Wazuh pipeline services")
...
func (m *ServicesModule) refreshServices() tea.Cmd {
	return tea.Cmd(func() tea.Msg {
		...
		for _, serviceName := range WazuhServices {
...
func (m *ServicesModule) getServiceDescription(serviceName string) string {
	descriptions := map[string]string{
		"wazuh-listener":       "Webhook receiver for Wazuh alerts",
		"wazuh-agent-enricher": "Agent metadata enrichment service",
