...
// configFile is the default location for the Wazuh agent configuration
const configFile = ".wazuh.json"

// ConfirmConfig prompts the user to confirm or modify configuration values
func ConfirmConfig(rc *eos_io.RuntimeContext, cfg *Config) *Config {
	// TODO: Implement interactive configuration confirmation
	// For now, just return the config as-is
	return cfg
}

// Authenticate authenticates with the Wazuh API and returns a token
func Authenticate(rc *eos_io.RuntimeContext, cfg *Config) (string, error) {
	authURL := fmt.Sprintf("%s://%s:%s/security/user/authenticate?raw=true",
...
// loadConfig reads the configuration from .wazuh.json.
...
// saveConfig writes the configuration back to .wazuh.json.
...
func queryUpgradeResult(rc *eos_io.RuntimeContext, apiURL, token string, agentIDs []string) error {
	...
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", token))
	tr := &http.Transport{TLSClientConfig: getWazuhTLSConfig()}
	// SECURITY: Set timeout to prevent resource exhaustion from slow/hanging connections
	client := &http.Client{
		Transport: tr,
...
// getWazuhTLSConfig returns TLS configuration with proper security settings for Wazuh/Wazuh API
func getWazuhTLSConfig() *tls.Config {
	...
	if os.Getenv("Eos_INSECURE_TLS") == "true" || os.Getenv("GO_ENV") == "test" {
		...
		_, _ = fmt.Fprintf(os.Stderr, "This makes Wazuh/Wazuh API connections vulnerable to MITM attacks. DO NOT USE IN PRODUCTION!\n")
		return &tls.Config{
	...
	// Secure TLS configuration for production Wazuh/Wazuh API connections
	...
	// SECURITY: Try to load custom CA certificate for self-signed Wazuh/Wazuh servers
