...
`, user.Username, user.Username, user.Username, user.HomeDir,
	...
	if err := os.WriteFile(instructionsPath, []byte(instructions), shared.ConfigFilePerm); err != nil {
...
`, strings.Join(auditRules, "\n"))
	...
	if err := os.WriteFile(rulesFile, []byte(content), shared.SecureConfigFilePerm); err != nil {
...
`
	...
	if err := os.WriteFile(limitsFile, []byte(limitsConfig), shared.ConfigFilePerm); err != nil {
