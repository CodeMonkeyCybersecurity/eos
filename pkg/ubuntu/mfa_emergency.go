...
case "$1" in
    enable)
        ...
        echo "⚠️  Please establish permanent access within this time!"
        echo "⚠️  Bypass will auto-disable or run: sudo emergency-mfa-bypass disable"
        ;;
...
func (m *MFAManager) rollback() error {
	if !m.rollbackEnabled {
		m.logger.Warn("⚠️ Rollback requested but not enabled")
		return nil
	}
...
`,
	...
	_ = os.WriteFile(notificationPath, []byte(notification), 0644)
