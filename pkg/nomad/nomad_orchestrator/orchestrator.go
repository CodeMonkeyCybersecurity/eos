...
func NewOrchestrator(rc *eos_io.RuntimeContext) *NomadOrchestrator {
	return &NomadOrchestrator{
		rc:          rc,
		nomadAddr:   fmt.Sprintf("http://%s:%d", shared.GetInternalHostname(), shared.PortNomad), // Default Nomad address
		consulAddr:  fmt.Sprintf("http://%s:%d", shared.GetInternalHostname(), shared.PortConsul),
