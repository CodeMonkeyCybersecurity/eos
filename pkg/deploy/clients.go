...
func NewSaltClient(config SaltClientConfig, httpClient HTTPClient) (*SaltClient, error) {
	return &SaltClient{
		config:     config,
...
// ExecuteState implements cicd.SaltClient interface
func (sc *SaltClient) ExecuteState(ctx context.Context, targets []string, state string, pillar map[string]interface{}) error {
	request := SaltJobRequest{
		Client:   "local",
		Target:   strings.Join(targets, ","),
		Function: "state.apply",
		Args:     []interface{}{state},
		Kwargs:   map[string]interface{}{"pillar": pillar},
		Timeout:  int(sc.config.Timeout.Seconds()),
	}

	reqBody, err := json.Marshal(request)
	if err != nil {
		return fmt.Errorf("failed to marshal Salt request: %w", err)
	}
	...
	if sc.config.Token != "" {
		headers["X-Auth-Token"] = sc.config.Token
	}

	resp, err := sc.httpClient.Post(ctx, sc.config.MasterURL, headers, reqBody)
	if err != nil {
		return fmt.Errorf("Salt API request failed: %w", err)
	}

	if resp.StatusCode != 200 {
		return fmt.Errorf("Salt API returned status %d: %s", resp.StatusCode, string(resp.Body))
	}

	var saltResp SaltJobResponse
	if err := json.Unmarshal(resp.Body, &saltResp); err != nil {
		return fmt.Errorf("failed to parse Salt response: %w", err)
	}

	if saltResp.Error != "" {
		return fmt.Errorf("Salt execution failed: %s", saltResp.Error)
	}

	return nil
}

// ExecuteOrchestrate implements cicd.SaltClient interface
func (sc *SaltClient) ExecuteOrchestrate(ctx context.Context, orchestrate string, pillar map[string]interface{}) error {
	request := SaltOrchestrationRequest{
		Client: "runner",
	...
	reqBody, err := json.Marshal(request)
	if err != nil {
		return fmt.Errorf("failed to marshal Salt orchestration request: %w", err)
	}
	...
	resp, err := sc.httpClient.Post(ctx, sc.config.MasterURL, headers, reqBody)
	if err != nil {
		return fmt.Errorf("Salt orchestration request failed: %w", err)
	}

	if resp.StatusCode != 200 {
		return fmt.Errorf("Salt orchestration returned status %d: %s", resp.StatusCode, string(resp.Body))
	}

	var saltResp SaltJobResponse
	if err := json.Unmarshal(resp.Body, &saltResp); err != nil {
		return fmt.Errorf("failed to parse Salt orchestration response: %w", err)
	}

	if saltResp.Error != "" {
		return fmt.Errorf("Salt orchestration failed: %s", saltResp.Error)
	}

	return nil
}

// GetJobStatus implements cicd.SaltClient interface
func (sc *SaltClient) GetJobStatus(ctx context.Context, jobID string) (*cicd.SaltJobStatus, error) {
	url := fmt.Sprintf("%s/jobs/%s", sc.config.MasterURL, jobID)
	...
	resp, err := sc.httpClient.Get(ctx, url, headers)
	if err != nil {
		return nil, fmt.Errorf("failed to get Salt job status: %w", err)
	}

	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("Salt API returned status %d", resp.StatusCode)
	}

	var jobInfo map[string]interface{}
	if err := json.Unmarshal(resp.Body, &jobInfo); err != nil {
		return nil, fmt.Errorf("failed to parse job status response: %w", err)
	}

	// Parse job status (simplified implementation)
	status := &cicd.SaltJobStatus{
		ID:     jobID,
		Status: "completed", // Would parse from actual response
		Result: jobInfo,
		Success: true,
		Duration: 30 * time.Second, // Would calculate from actual times
	}

	return status, nil
}

// Ping implements cicd.SaltClient interface
func (sc *SaltClient) Ping(ctx context.Context, targets []string) error {
	return sc.ExecuteState(ctx, targets, "test.ping", nil)
}

// UpdatePillar updates Salt pillar data
func (sc *SaltClient) UpdatePillar(ctx context.Context, pillarName string, data map[string]interface{}) error {
	// Implementation would write pillar data to Salt pillar directory or API
