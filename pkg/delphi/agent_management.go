// pkg/delphi/agent_management.go
//
// Delphi Agent Management System
//
// This package provides comprehensive Delphi agent management including upgrade,
// re-registration, and analysis capabilities. Since Delphi is your own implementation
// of Wazuh, this system handles both Delphi and Wazuh agents interchangeably.
//
// Key Features:
// - Local agent upgrade with comprehensive pre-flight checks
// - Agent re-registration with new Delphi servers
...
// - Integration with existing Delphi infrastructure
//
// Use Cases:
// - Upgrading agents from older versions (v4.10.1-v4.12.0 → v4.13.0)
// - Re-registering agents after Delphi server replacement
...
// UpgradeLocalAgent performs local Delphi agent upgrade with comprehensive pre-flight checks
...
// analyzeLocalAgent performs comprehensive analysis of the local Delphi agent
func (aum *AgentUpgradeManager) analyzeLocalAgent(rc *eos_io.RuntimeContext) (*AgentAnalysis, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get current Delphi agent version: %w", err)
	}
	...
	if targetVersion == "" {
		...
		latestVersion, err := GetLatestDelphiVersion(rc)
		if err != nil {
			logger.Warn("Failed to get latest version, using default", zap.Error(err))
			targetVersion = DefaultDelphiVersion
		} else {
			...
			for _, line := range lines {
				if strings.Contains(line, "wazuh-agent") {
					...
					if len(fields) >= 3 {
						...
						logger.Debug("Found Delphi agent version via dpkg", zap.String("version", version))
						return version, nil
					}
				}
			}
			...
			for _, line := range lines {
				if strings.Contains(strings.ToLower(line), "version") {
					...
					if len(parts) >= 2 {
						...
						logger.Debug("Found Delphi agent version via wazuh-control", zap.String("version", version))
						return version, nil
					}
				}
			}
		...
		if output, err := execute.Run(rc.Ctx, execute.Options{
			...
			for _, line := range lines {
				if strings.Contains(strings.ToLower(line), "version") {
					...
					if len(parts) >= 2 {
						version := strings.TrimSpace(parts[1])
						logger.Debug("Found Delphi agent version on macOS", zap.String("version", version))
						return version, nil
					}
				}
			}
