	...
	if interaction.PromptYesNo(rc.Ctx, "Enable Multi-Factor Authentication (MFA)?", true) {
		...
		if userpassConfigured {
			...
			if err := SetupUserTOTP(rc, client, "eos", bootstrapData); err != nil {
				...
				log.Error("")
				log.Error("To retry: Run 'eos create vault' and answer 'Yes' to MFA prompt")
				log.Error("Or use diagnostics: 'eos debug vault --mfa' to troubleshoot")
				log.Error("")
			...
			if err := EnforceMFAPolicyOnly(rc, client, mfaConfig, bootstrapData.EntityID); err != nil {
				...
				log.Error("")
				log.Error("To retry enforcement: Run 'eos update vault --fix --mfa' to repair MFA policies")
				log.Error("Or use diagnostics: 'eos debug vault --mfa' to diagnose the issue")
				log.Error("")
			...
			log.Warn("  Run 'eos create vault' again and answer 'Yes' to MFA prompt")
			log.Warn("")
		}
