...
// GetVaultClient returns a cached or freshly initialized Vault client, validating its health.
func GetVaultClient() (*api.Client, error) {
	...
	client, err := newClientFromEnvOrFallback()
	if err != nil {
		return nil, err
	}
...
func newClientFromEnvOrFallback() (*api.Client, error) {
	if client, err := newClientWithEnv(); err == nil {
		return client, nil
	}
	zap.L().Warn("⚠️ Environment client failed, trying privileged fallback")

	client, err := newPrivilegedClient()
	if err != nil {
		return nil, fmt.Errorf("failed to create privileged Vault client: %w", err)
	}
	return client, nil
}
