...
- **Smallest change**: 4 targeted edits — 2 e2e test scripts, golangci.yml (noceph tag + timeout), CI workflow (apt-get install), lint.sh (remove hardcoded --timeout=8m)
- **Idempotency**: all fixes are re-entrant; `pip install` is idempotent, `apt-get install` is idempotent, build tags are additive
- **Risk + rollback**: low risk; test changes make tests more portable; CGo lib install adds ~30s to CI but is the correct root-cause fix
