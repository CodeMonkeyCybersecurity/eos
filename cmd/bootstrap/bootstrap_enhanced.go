...
// bootstrapAdditionalNode handles joining additional nodes to an existing HashiCorp cluster
func bootstrapAdditionalNode(rc *eos_io.RuntimeContext, cmd *cobra.Command, clusterInfo *bootstrap.ClusterInfo) error {
	...
	healthResult, err := bootstrap.PerformHealthChecks(rc, "localhost:8500") // HashiCorp Consul address
	if err != nil {
	...
	// Phase 1: Register with HashiCorp cluster
	consulAddr := "localhost:8500" // Default Consul address, should be configurable
	logger.Info("Phase 1: Registering with HashiCorp cluster",
		zap.String("consul_addr", consulAddr),
		zap.String("cluster_id", clusterInfo.ClusterID))

	result, err := bootstrap.RegisterNode(rc, consulAddr, reg)
	if err != nil {
	...
	// Phase 4: Apply  highstate to configure node
	logger.Info("Phase 4: Applying  highstate for node configuration")
...
func showPostBootstrapInfo(logger otelzap.LoggerWithCtx, info *bootstrap.ClusterInfo) {
	...
	if info.IsSingleNode {
		...
		if info.MyRole == environment.RoleMonolith {
			...
			logger.Info("This node has joined the HashiCorp cluster")
			logger.Info("Cluster ID: " + info.ClusterID)
		}
	}
...
func createEnhancedEnvironmentConfig(rc *eos_io.RuntimeContext, clusterInfo *bootstrap.ClusterInfo) error {
	...
	// Following the  to HashiCorp migration, environment discovery
	// will be handled through Consul service discovery instead of  s
