...
var pipelinePromptsCmd = &cobra.Command{
	Use:   "pipeline-prompts <prompt-name>",
...
Examples:
  eos delete pipeline-prompts my-custom-prompt
  eos delete pipeline-prompts old-prompt --force
  eos delete pipeline-prompts temp-prompt --backup --force`,
...
func init() {
	pipelinePromptsCmd.Flags().BoolP("force", "f", false, "Skip confirmation prompt")
	pipelinePromptsCmd.Flags().BoolP("backup", "b", false, "Create backup before deletion")

	DeleteCmd.AddCommand(pipelinePromptsCmd)
}
