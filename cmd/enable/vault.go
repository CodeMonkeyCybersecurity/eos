...
AppRole, userpass, and creates an eos user with a random password.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Println("[9/10] Ensuring Linux system user 'eos' exists...")
		check := exec.Command("id", "-u", "eos")
		if err := check.Run(); err == nil {
			fmt.Println("✅ Linux user 'eos' already exists.")
		} else {
			create := exec.Command("useradd", "--system", "--create-home", "--shell", "/usr/sbin/nologin", "--user-group", "eos")
			if err := create.Run(); err != nil {
				log.Fatal("Failed to create Linux user 'eos'", zap.Error(err))
			}
			fmt.Println("✅ Created Linux user 'eos'")
		}

		// 10. Generate a random password and create an eos user with it
		...
		fmt.Println("\nEos priviliged user created successfully with userpass auth.")
