...
var DockerListenerCmd = &cobra.Command{
	...
	RunE: eos.Wrap(func(ctx *eos_io.RuntimeContext, cmd *cobra.Command, args []string) error {
		...
		steps := []struct {
			...
			{"🔧 apt update", func() error { _, err := execute.RunShell("apt update"); return err }},
			{"🔧 install python3-venv + pip", func() error {
				_, err := execute.RunShell("apt install -y python3-venv python3-pip")
				return err
			}},
			{"📂 create venv dir", func() error {
				return execute.RunSimple("mkdir", "-p", shared.VenvPath)
			}},
			{"🐍 create venv", func() error {
				return execute.RunSimple("python3", "-m", "venv", shared.VenvPath)
			}},
			{"📦 pip install requirements", func() error {
				return execute.RunSimple(shared.VenvPath+"/bin/pip", "install",
					"docker==7.1.0", "urllib3==1.26.20", "requests==2.32.2")
			}},
			{"✏️ patch DockerListener", patchDockerListener},
			{"🔄 restart wazuh-agent", func() error {
				return execute.RunSimple("systemctl", "restart", "wazuh-agent")
			}},
		}
		...
		zap.L().Info("✅ DockerListener setup complete.")
		return nil
	}),
}

func patchDockerListener() error {
	path := shared.DockerListener
	if _, err := os.Stat(path); os.IsNotExist(err) {
		zap.L().Warn("⚠️ DockerListener script not found", zap.String("path", path))
		return nil
	}

	backup := path + ".bak"
	if err := execute.RunSimple("cp", path, backup); err != nil {
		zap.L().Warn("⚠️ Failed to backup DockerListener", zap.Error(err))
	}
	...
	zap.L().Info("✅ DockerListener script patched", zap.String("path", path))
	return nil
}
