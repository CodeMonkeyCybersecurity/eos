...
FROM processes 
WHERE name IN ('salt-master', 'salt-minion', 'vault', 'nomad', 'consul', 'docker', 'osqueryd', 'clusterfuzz')
ORDER BY name;`
...
FROM deb_packages 
WHERE name IN ('salt-master', 'salt-minion', 'salt-common', 'vault', 'nomad', 'consul', 'docker-ce', 'osquery')
ORDER BY name;`
...
FROM systemd_units 
WHERE name IN ('salt-master.service', 'salt-minion.service', 'vault.service', 'nomad.service', 'consul.service', 'docker.service', 'osqueryd.service')
ORDER BY name;`
...
FROM file 
WHERE path IN ('/opt/eos', '/srv/salt', '/etc/salt', '/opt/vault', '/opt/nomad', '/etc/vault.d', '/etc/nomad.d', '/opt/clusterfuzz')
ORDER BY path;`
...
WHERE name IN ('salt-master', 'salt-minion', 'vault', 'nomad', 'consul', 'osqueryd');`
...
WHERE name IN ('salt-master', 'salt-minion', 'vault', 'nomad', 'consul', 'osquery');`
