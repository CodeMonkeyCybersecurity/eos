...
func runCheck(rc *eos_io.RuntimeContext, cmd *cobra.Command, args []string) error {
	...
	if pathEos, err := exec.LookPath("eos"); err == nil {
		...
		if verbose && pathEos != execPath {
			logger.Info("terminal prompt:   ⚠️  Note: Different from current executable")
		}
	} else {
		warnings = append(warnings, "eos not found in PATH")
		logger.Info("terminal prompt:   ⚠️  Not found in PATH")
		if verbose {
	...
	if wd, err := os.Getwd(); err == nil {
		...
		if info, err := os.Stat(localEos); err == nil && !info.IsDir() {
			if info.Mode()&0111 == 0 {
				...
				logger.Info("terminal prompt:   ⚠️  Local eos binary found but not executable")
			} else {
	...
	for _, path := range commonPaths {
		if info, err := os.Stat(path); err == nil && !info.IsDir() {
			if info.Mode()&0111 != 0 {
				...
				logger.Info("terminal prompt:   ⚠️  Found but not executable: "+path)
			}
		} else if verbose {
