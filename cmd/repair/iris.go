...
var irisRepairCmd = &cobra.Command{
	Use:   "iris",
	Short: "Repair and fix Iris installation issues",
...
func init() {
	irisRepairCmd.Flags().BoolVar(&dryRun, "dry-run", false, "Show what would be fixed without making changes")
	irisRepairCmd.Flags().BoolVar(&autoYes, "yes", false, "Automatically approve all fixes")
	irisRepairCmd.Flags().BoolVar(&interactive, "interactive", true, "Ask for confirmation before each fix (default)")
}
