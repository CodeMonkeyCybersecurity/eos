...
var vaultRepairCmd = &cobra.Command{
	Use:   "vault",
	Short: "Repair common Vault installation issues",
...
func init() {
	vaultRepairCmd.Flags().BoolVar(&vaultRepairDryRun, "dry-run", false, "Show what would be fixed without making changes")
	vaultRepairCmd.Flags().BoolVar(&vaultRepairCleanupBinaries, "cleanup-binaries", false, "Remove duplicate vault binaries")
	vaultRepairCmd.Flags().BoolVar(&vaultRepairPermissions, "permissions", false, "Fix file permissions and ownership")
	vaultRepairCmd.Flags().BoolVar(&vaultRepairConfig, "config", false, "Repair configuration files")
	vaultRepairCmd.Flags().BoolVar(&vaultRepairAll, "all", false, "Repair all detected issues")
...
func runVaultRepair(rc *eos_io.RuntimeContext, cmd *cobra.Command, args []string) error {
	...
	if issuesFound == 0 {
		...
		fmt.Printf("\n⚠️  %d issues could not be automatically repaired\n", issuesFound-issuesFixed)
...
func repairFilePermissions(rc *eos_io.RuntimeContext, dryRun bool) (int, int, error) {
	...
	for _, check := range fileChecks {
		...
		if actualPerm != check.expectedPerm {
			issuesFound++
			fmt.Printf("⚠️  %s has incorrect permissions: %o (expected %o)\n",
...
func repairConfiguration(rc *eos_io.RuntimeContext, dryRun bool) (int, int, error) {
	...
	fmt.Println()
	fmt.Printf("⚠️  Configuration has %d errors:\n", issuesFound)
