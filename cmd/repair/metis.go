...
var metisRepairCmd = &cobra.Command{
	Use:   "metis",
	Short: "Repair and fix Metis installation issues",
...
func init() {
	metisRepairCmd.Flags().BoolVar(&dryRun, "dry-run", false, "Show what would be fixed without making changes")
	metisRepairCmd.Flags().BoolVar(&autoYes, "yes", false, "Automatically approve all fixes")
	metisRepairCmd.Flags().BoolVar(&interactive, "interactive", true, "Ask for confirmation before each fix (default)")
}
