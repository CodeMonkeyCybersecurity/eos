...
func runOpenWebUIDebug(rc *eos_io.RuntimeContext, cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		fmt.Printf("  ✗ Failed to pull alpine image: %s\n", output)
		return fmt.Errorf("cannot pull alpine image: %w", err)
	}
	fmt.Println("  ✓ Alpine image available")
	fmt.Println()
	...
	if err != nil {
		...
		logger.Error("Basic backup failed", zap.String("output", output), zap.Error(err))
	...
	if err != nil {
		...
		logger.Warn("Secure backup with :ro failed", zap.String("output", output), zap.Error(err))
	} else {
	...
	if err != nil {
		...
		logger.Error("Secure backup without :ro failed", zap.String("output", output), zap.Error(err))
