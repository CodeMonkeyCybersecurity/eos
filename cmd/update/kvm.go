...
func init() {
	// Add update kvm command (VM shell access)
	updateKvmCmd.Flags().BoolVar(&kvmEnableGuestExec, "enable-guest-exec", false, "Enable QEMU guest agent guest-exec commands")
	updateKvmCmd.Flags().BoolVar(&kvmAllDisabledGuestExec, "all-disabled", false, "Enable guest-exec for all disabled VMs")
	updateKvmCmd.Flags().BoolVar(&kvmYesConfirm, "yes", false, "Skip confirmation prompts")

	UpdateCmd.AddCommand(updateKvmCmd)

	// Add restart kvm command
	restartKvmCmd.Flags().BoolVar(&kvmNoSafe, "no-safe", false, "Skip safety checks (dangerous!)")
