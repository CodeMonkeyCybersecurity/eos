...
Environment Registration:
  --environment ENV        Set node's environment in Consul KV (development, staging, production, review)

  The --environment flag registers this node's environment in Consul KV at
  eos/nodes/{hostname}/environment. This enables automatic environment detection
  for Vault secret operations without manual --environment flags.

  Consul becomes the authoritative source of truth for node environment.

  Workflow:
  1. Set environment once: eos update consul --environment production
  2. Vault secret commands auto-detect environment from Consul
  3. No need to specify --environment on every command

  Production changes require interactive confirmation for safety.

  Examples:
    eos update consul --environment development
    eos update consul --environment production  # Requires confirmation
...
Examples:
  # Register node environment (one-time setup)
  eos update consul --environment development
  eos update consul --environment production  # Requires confirmation
...
func init() {
	ConsulCmd.Flags().StringVar(&consulEnvironment, "environment", "",
		"Set node's environment in Consul KV (development, staging, production, review)")
...
func runConsulUpdate(rc *eos_io.RuntimeContext, cmd *cobra.Command, args []string) error {
	...
	if operationCount > 1 {
		return eos_err.NewUserError(
			"Cannot specify multiple operations simultaneously.\n\n" +
				"Choose ONE of:\n" +
				"  --environment      Set node's environment in Consul KV\n" +
	...
	// Handle --environment flag (environment registration)
	if consulEnvironment != "" {
		logger.Info("Registering node environment in Consul KV",
			zap.String("environment", consulEnvironment))
		...
		if err != nil {
			return fmt.Errorf("failed to connect to Consul: %w\n\n"+
				"Remediation:\n"+
				"  - Ensure Consul is running: systemctl status consul\n"+
				"  - Check Consul health: consul members", err)
		}

		// Check if environment already set (NOT idempotent - user requirement)
		kvPath := fmt.Sprintf("eos/nodes/%s/environment", hostname)
		existing, _, err := consulClient.KV().Get(kvPath, nil)
		if err != nil {
		...
		if consulEnvironment == string(sharedvault.EnvironmentProduction) {
			...
			logger.Info("Ensure this is correct before proceeding")

			confirmed := promptConfirm("Set environment to PRODUCTION? (y/N): ")
			if !confirmed {
		...
		logger.Info("Environment registered successfully",
			...
			zap.String("environment", consulEnvironment),
			zap.String("consul_path", kvPath))
		logger.Info("Vault secret commands will now auto-detect this environment")
		logger.Info("No need to specify --environment flag on secret operations")
	...
	if consulPorts == "" {
		return eos_err.NewUserError(
			"Must specify one of: --environment, --ports, --fix, --bootstrap-token, or --enable-acls.\n\n" +
