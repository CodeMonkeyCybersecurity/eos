...
The Nomad-based vault integration provides the same capabilities but with simpler orchestration.`,
	...
	RunE: eos.Wrap(func(rc *eos_io.RuntimeContext, cmd *cobra.Command, args []string) error {
		...
		logger.Info(fmt.Sprintf("terminal prompt:  K3s configuration with Vault integration generated in: %s", outputDir))
		...
		logger.Info(fmt.Sprintf("terminal prompt:    3. Deploy with: eos create terraform-vault %s --vault-secrets", outputDir))
...
var generateVaultHetznerCmd = &cobra.Command{
	...
	RunE: eos.Wrap(func(rc *eos_io.RuntimeContext, cmd *cobra.Command, args []string) error {
		...
		logger.Info(fmt.Sprintf("terminal prompt:  Hetzner configuration with Vault integration generated in: %s", outputDir))
