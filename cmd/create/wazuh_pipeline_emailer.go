...
var deployTemplateCmd = &cobra.Command{
	Use:   "deploy-template",
	Short: "Deploy email template for Wazuh emailer service",
	Long: `Deploy the email template required by the wazuh-emailer service.

This command:
- Creates the /opt/stackstorm/packs/wazuh/ directory structure
- Copies the email.html template from assets/ to the target location
- Sets correct permissions (stanley:stanley 0644)
- Ensures the template is accessible by the wazuh-emailer service

The template will be deployed to: /opt/stackstorm/packs/wazuh/email.html`,
	RunE: eos.Wrap(func(rc *eos_io.RuntimeContext, cmd *cobra.Command, args []string) error {
		logger := otelzap.Ctx(rc.Ctx)
		logger.Info(" Deploying email template for Wazuh emailer service")
		...
		targetDir := "/opt/stackstorm/packs/wazuh"
		...
		if _, err := os.Stat(sourcePath); os.IsNotExist(err) {
			logger.Error(" Source template not found",
				...
				zap.Error(err))
			logger.Info(" The email template should be deployed during 'eos create wazuh'")
			return err
		}
		...
		logger.Info("   1. Ensure .env file is configured at /opt/stackstorm/packs/wazuh/.env")
		logger.Info("   2. Restart emailer service: eos wazuh services restart wazuh-emailer")
		logger.Info("   3. Check service status: eos wazuh services status wazuh-emailer")
...
func init() {
	CreateCmd.AddCommand(deployTemplateCmd)
}
